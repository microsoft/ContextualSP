    ...
    def _process_root(self, sql_clause: Dict) -> List[Action]:
        ...
        def _use_sep_join(_sql_clause: Dict) -> bool:
            ...
            for table_unit_tuple in _sql_clause['from']['table_units']:
                # table unit tuple[1] is the table id
                if type(table_unit_tuple[1]) == int and self.table_names[table_unit_tuple[1]].name in infer_tables:
            ...
            if len(join_tables) == 2 and len(used_tables) == 1 and len(set(join_tables) - used_tables):
        ...
        if filter_used and order_used:
            if join_used:
                inter_seq, next_states = [Root(GrammarType.RootJSFO)], ['Join', 'Select', 'Filter', 'Order']
            else:
                inter_seq, next_states = [Root(GrammarType.RootSFO)], ['Select', 'Filter', 'Order']
        elif filter_used:
            if join_used:
                inter_seq, next_states = [Root(GrammarType.RootJSF)], ['Join', 'Select', 'Filter']
            else:
                inter_seq, next_states = [Root(GrammarType.RootSF)], ['Select', 'Filter']
        elif order_used:
            if join_used:
                inter_seq, next_states = [Root(GrammarType.RootJSO)], ['Join', 'Select', 'Order']
            else:
...
class ActionConverter(object):
    ...
    def _process_join(self, component_mapping: Dict[str, List[ActionTreeNode]],
                      repr: Dict[str, str],
                      is_subquery: bool):
        """
    ...
    def _process_sep_join(self, node: Optional[ActionTreeNode], component: Dict[str, List[ActionTreeNode]],
                          repr: Dict[str, str]):
        """
    ...
    def _build_graph(self):
        ...
        for key_full_name in self.db_context.knowledge_graph.neighbors_with_table.keys():
            # get key name
            key_tab_name, key_col_name = _get_tab_col_name(key_full_name)
            if key_col_name is None:
                continue
            linked_value_set = self.db_context.knowledge_graph.neighbors_with_table[key_full_name]
