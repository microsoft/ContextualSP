...
from models.mbart.mbartForST import MBartForST

from train_utils.trainer import Seq2SeqRATTrainer

MODEL_CLASS = {
    "M2M100ForST": M2M100ForST,
    "MBartForST": MBartForST,
}
...
def main():
    ...
    if not "rat" in str(model_args.model_class).lower():
    ...
    #
    #     metrics = train_utils.evaluate(
    #         max_length=data_args.val_max_target_length, num_beams=data_args.num_beams, metric_key_prefix="eval"
    #     )
    #     max_val_samples = data_args.max_val_samples if data_args.max_val_samples is not None else len(eval_dataset)
    #     metrics["eval_samples"] = min(max_val_samples, len(eval_dataset))
    #
    #     train_utils.log_metrics("eval", metrics)
    #     train_utils.save_metrics("eval", metrics)
    if training_args.do_eval:
