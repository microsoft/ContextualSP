...
class Encoder(nn.Module):
    ...
    def forward(self, x, relation, types, mask):
        "Pass the input (and mask) through each layer in turn."
        for layer in self.layers:
            x = layer(x, relation, types, mask)
        return self.norm(x)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, num_structure_kinds, num_type_kinds, dropout):
        ...
        self.relation_k_emb = nn.Embedding(num_structure_kinds, self.self_attn.d_k)
        self.relation_v_emb = nn.Embedding(num_structure_kinds, self.self_attn.d_k)

        self.types_k_emb = nn.Embedding(num_type_kinds, self.self_attn.d_k)
        self.types_v_emb = nn.Embedding(num_type_kinds, self.self_attn.d_k)
    ...
    def forward(self, x, relation, types, mask):
        ...
        types_k = self.types_k_emb(types)
        types_v = self.types_v_emb(types)

        relation_k += types_k
        relation_v += types_v

        relation_k = self.dropout_k(relation_k)
        relation_v = self.dropout_k(relation_v)
