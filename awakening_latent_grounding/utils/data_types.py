...
class SpiderSchema:
    ...
    def to_string(self, sep: str = '\n'):
        ...
        for c_idx1, c_idx2 in self.foreign_keys:
            fk_strs.append('{}::{} - {}::{}'.format(self.table_names[self.column_to_table[c_idx1]], self.column_names[c_idx1], self.table_names[self.column_to_table[c_idx2]], self.column_names[c_idx2]))
...
class ColumnToken(SQLToken):
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, ColumnToken) and other.column_name == self.column_name and other.suffix_type == self.suffix_type
...
class SQLTokenHypothesis:
    ...
    def update(self, token: SQLToken, score: float):
        return SQLTokenHypothesis(tokens=self.tokens + [token], scores=self.scores + [score], total_score=self.total_score + score)
...
class AlignmentLabel:
    ...
    def to_json(self) -> Dict:
        return { 'token': self.token.to_json(), 'align_type': self.align_type, 'align_value': self.align_value, 'confidence': self.confidence }
...
def generate_utterance(tokenizer: BertTokenizer, text: str, tokens: List[str] = None, lemma: List[str] = None) -> Utterance:
