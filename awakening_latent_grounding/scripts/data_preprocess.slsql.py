...
logging.basicConfig(filename=os.path.join(data_dir, 'preprocess.log'), level=logging.DEBUG)


# %%
# load schemas from database
def get_column_names_unique(column_names: List[Tuple[int, str]], table_names: List[str], primary_keys: List[int]) -> \
List[str]:
...
def get_column_name_normalized(column_lem_names: List[Tuple[int, str]], table_lem_names: List[str],
                               verbose: bool = False):
    ...
    with open(path, 'r', encoding='utf-8') as fr:
        for line in fr:
            ...
            for column in table['columns']:
                column = (
                "{}.{}".format(table_name, column['column_name']).lower(), column['data_type'], column['values'])
...
value_matchers = load_value_matches(os.path.join(data_dir, 'spider_tables.txt'))
...
def _is_in_column(idx, field: str, tokens: List[SQLToken]):
    ...
    if field.lower() == 'select':
        if idx - 2 >= 0 and tokens[idx - 2].token_type == SQLTokenType.keyword and tokens[
            idx - 2].value.lower() == 'in':
            return True

        # SELECT singer.name FROM singer WHERE singer.singer_id NOT IN ( SELECT song.singer_id FROM song )
        if idx - 3 >= 0 and tokens[idx - 3].token_type == SQLTokenType.keyword and tokens[
            idx - 3].value.lower() == 'in':
            return True
...
def _is_group_by_key_column(idx, field: str, tokens: List[SQLToken], schema: SpiderSchema):
    if field.lower() == 'group':
        if isinstance(tokens[idx], ColumnToken):
            key_code = schema.get_column_key_code(schema.id_map[tokens[idx].column_name])
            if key_code != 0:
                return True
    return False
...
def generate_identify_labels_from_sql(sql: SQLExpression, schema: SpiderSchema):
    ...
    for i, token in enumerate(sql.tokens):
        if isinstance(token, KeywordToken):
            ...
            if not is_from and token.column_name != '*' and not _is_in_column(i, field,
                                                                              sql.tokens) and not _is_group_by_key_column(
                    i, field, sql.tokens, schema):
            ...
            if not is_from and field != 'LIMIT':
                ...
                identify_labels[str(SQLTokenType.value)].append((token.value, token.columns))
        else:
    ...
    for count in value2column_count.values():
        statistics['max_value_count'] = max(statistics['max_value_count'], count)
...
def generate_identify_labels_from_align(ant: Dict, schema: SpiderSchema):
    ...
    for tok_idx, tok_ant in enumerate(ant):
        ...
        if e_type == 'tbl':
            identify_labels[str(SQLTokenType.table)].append(schema.table_names_original[e_idx].lower())
        elif e_type == 'col':
            identify_labels[str(SQLTokenType.column)].append(schema.get_column_full_name(e_idx))
        elif e_type == 'val':
            identify_labels[str(SQLTokenType.value)].append('val_{}'.format(schema.get_column_full_name(e_idx)))
...
def generate_masking_ngrams(question: Utterance, schema: SpiderSchema) -> List[Tuple[int, int, str]]:
    if schema.db_id not in ngram_matchers:
        ...
        for i, column in enumerate(schema.column_names):
            column_tokens.append((schema.get_column_full_name(i), column.split(' ')))
        for i, table in enumerate(schema.table_names):
            column_tokens.append((schema.table_names_original[i], table.split(' ')))
...
def resolve_values(question: Utterance, schema: SpiderSchema, sql: SQLExpression):
    ...
    for token in sql.tokens:
        if isinstance(token, ValueToken) and len(token.columns) > 0:
            ...
            for column in token.columns:
                values_dict[(str(token.value).strip("\"").strip('%').lower(), column.lower())] = False
    ...
    for (value, column), resolved in values_dict.items():
        if not resolved:
            all_resolved = False
            logging.info('Value resolved: {}/{}/{}\t{}'.format(value, schema.db_id, column, question.text))
...
def process_squall_query(query: Dict):
    ...
    question_utterance = generate_utterance(tokenizer, question, [fix_tok(x) for x in query['toks']],
                                            [fix_tok(x) for x in query['lemma']])
    ...
    value_resolved, matched_values = resolve_values(question_utterance, schema, sql)
...
sql = parse_spider_sql(
    ...
    schemas['pets_1'])
# %%
value_matchers['car_1'].match(
    'What are the ids and names of all countries that either have more than 3 car makers or produce fiat model ?'.split(),
    0.8)
# %%
dev_queries = json.load(open(os.path.join(data_dir, 'slsql_dev.json'), 'r', encoding='utf-8'))
train_queries = json.load(open(os.path.join(data_dir, 'slsql_train.json'), 'r', encoding='utf-8'))
...
print('process dev over, value_unresolved: {}'.format(statistics['value_unresolved']))

open(os.path.join(data_dir, 'dev.parsed_sqls.log'), 'w', encoding='utf-8').write('\n'.join(sql_logs))
...
def _compare_identify_labels(example: Dict):
    ...
    identify_labels_from_align = generate_identify_labels_from_align(example['align_labels'], schema)
    cmp_results = []
    cmp_results.append("Q: {}\n".format(question.text))
    cmp_results.append("SQL: {}\n".format(sql.sql))
    cmp_results.append("Table Shared: {}\n".format(' '.join(sorted(
        set(identify_labels_from_sql[str(SQLTokenType.table)]) & set(
            identify_labels_from_align[str(SQLTokenType.table)])))))
    cmp_results.append("Table SQL: {}\n".format(' '.join(sorted(
        set(identify_labels_from_sql[str(SQLTokenType.table)]) - set(
            identify_labels_from_align[str(SQLTokenType.table)])))))
    cmp_results.append("Table Align: {}\n".format(' '.join(sorted(
        set(identify_labels_from_align[str(SQLTokenType.table)]) - set(
            identify_labels_from_sql[str(SQLTokenType.table)])))))
    cmp_results.append("Column Shared: {}\n".format(' '.join(sorted(
        set(identify_labels_from_sql[str(SQLTokenType.column)]) & set(
            identify_labels_from_align[str(SQLTokenType.column)])))))
    cmp_results.append("Column SQL: {}\n".format(' '.join(sorted(
        set(identify_labels_from_sql[str(SQLTokenType.column)]) - set(
            identify_labels_from_align[str(SQLTokenType.column)])))))
    cmp_results.append("Column Align: {}\n".format(' '.join(sorted(
        set(identify_labels_from_align[str(SQLTokenType.column)]) - set(
            identify_labels_from_sql[str(SQLTokenType.column)])))))
    cmp_results.append("Table: {}\n".format(set(identify_labels_from_sql[str(SQLTokenType.table)]) == set(
        identify_labels_from_align[str(SQLTokenType.table)])))
    cmp_results.append("Column: {}\n".format(set(identify_labels_from_sql[str(SQLTokenType.column)]) == set(
        identify_labels_from_align[str(SQLTokenType.column)])))
    cmp_results.append('\n')

    identify_labels_equal['table'] += int(set(identify_labels_from_sql[str(SQLTokenType.table)]) == set(
        identify_labels_from_align[str(SQLTokenType.table)]))
    identify_labels_equal['column'] += int(set(identify_labels_from_sql[str(SQLTokenType.column)]) == set(
        identify_labels_from_align[str(SQLTokenType.column)]))
    return cmp_results
...
compare_identify_labels(dev_processed, os.path.join(data_dir, 'dev.identify_labels.diff.txt'))
...
save_json_objects(train_processed, os.path.join(data_dir, 'train.{}.json'.format(bert_version)))
print('process train over, value_unresolved: {}'.format(statistics['value_unresolved']))
# %%
dev_iter = load_spider_data_iterator(os.path.join(data_dir, 'dev.{}.json'.format(bert_version)), tokenizer, 16,
                                     torch.device('cpu'), False, False, 512)
# %%
...
for batch_input in dev_iter:
    ...
    num_examples += bs
    for i in range(bs):
        input_tokens.append(" ".join(batch_input['input_tokens'][i]) + '\n')
print(total_size, num_examples, total_size / num_examples)
open(os.path.join(data_dir, 'dev.input_tokens.txt'), 'w', encoding='utf-8').writelines(input_tokens)
# %%
train_iter = load_spider_data_iterator(os.path.join(data_dir, 'train.{}.json'.format(bert_version)), tokenizer, 16,
                                       torch.device('cpu'), True, True, 512)
total_size, num_examples = 0, 0
...
print(total_size, num_examples, total_size / num_examples)
# %%
train_iter2 = load_spider_data_iterator(os.path.join(data_dir, 'train.{}.json'.format(bert_version)), tokenizer, 16,
                                        torch.device('cpu'), False, True, 400)
total_size, num_examples = 0, 0
...
print(total_size, num_examples, total_size / num_examples)
# %%
dev_examples = json.load(open(os.path.join(data_dir, 'train.{}.json'.format(bert_version)), 'r', encoding='utf-8'))
threshold = 0.81
