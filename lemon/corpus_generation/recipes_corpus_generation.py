...
parser.add_argument("--corpus_file", type=str, default='../corpus/pretraining_corpus_recipes.txt', help="corpus file")
...
if __name__ == '__main__':
    ...
    data_lines = json.load(open('./recipes_train.json', 'r'))
    id_location_dict = dict()
    id_sentence_dict = dict()
    for line in data_lines:

        locations = line['gold_loc_seq']
        sentence_list = [item['sentence'] for item in line['sentence_list']]
        entity = line['entity']
        id = line['id']
        
        if id not in id_sentence_dict:
            id_sentence_dict[id] = sentence_list
        else:
            assert id_sentence_dict[id] == sentence_list
        

        if id not in id_location_dict:
            id_location_dict[id] = [(entity, locations)]
        else:
            id_location_dict[id].append((entity, locations))

    candidate_list = []
    for id in id_location_dict:
        sentence_list = id_sentence_dict[id]
        location_list = id_location_dict[id]

        participants = [item[0] for item in location_list]
        states = [item[1] for item in location_list]
